...
def draw_vertical_line(img, start_pt, end_pt, clr, thickness):
    n_steps = 7
    ...
    cv2.line(img, (int(start_pt[0]), int(start_pt[1])), (int(pt1[0]), int(pt1[1])), clr, thickness)
    cv2.line(img, (int(pt4[0]), int(pt4[1])), (int(end_pt[0]), int(end_pt[1])), clr, thickness)
